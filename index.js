...
function parseMessage(msg, aAdmin) {
    ...
    // settings menu
        ...
        sendMessageByBot(messageChatId, '?? Settings Menu',
    ...
    mongoOperationsTable.aggregate(query).toArray()
        ...
        mongoOperationsTable.aggregate(query).toArray()
            ...
            if (result.length) {
                ...
                message += `1?? ${dbItem._id.unm} spent ${val} ${tokenName}\n`;
                if (result.length > 1) {
                    dbItem = result[1];
                    val = coinFormat(dbItem.pp, 4);
                    message += `2?? ${dbItem._id.unm} spent ${val} ${tokenName}\n`;
                }
                if (result.length > 2) {
                    dbItem = result[2];
                    val = coinFormat(dbItem.pp, 4);
                    message += `3?? ${dbItem._id.unm} spent ${val} ${tokenName}\n`;
                }
            } else {
               ...
               // query Recieved most
                ...
                mongoOperationsTable.aggregate(query).toArray()
                    ...
                    if (result.length) {
                        ...
                        message += `1?? ${dbItem._id.unm} received ${val} ${tokenName}\n`;
                        if (result.length > 1) {
                            dbItem = result[1];
                            val = coinFormat(dbItem.pp, 4);
                            message += `2?? ${dbItem._id.unm} received ${val} ${tokenName}\n`;
                        }
                        if (result.length > 2) {
                            dbItem = result[2];
                            val = coinFormat(dbItem.pp, 4);
                            message += `3?? ${dbItem._id.unm} received ${val} ${tokenName}\n`;
                        }
                    } else {
                    ...
                    mongoOperationsTable.aggregate(query).toArray()
                        ...
                        if (result.length) {
                            ...
                            message += '1?? ' + dbItem._id.unm + ' was tipped ' + dbItem.count + ' times\n';
                            if (result.length > 1) {
                                dbItem = result[1];
                                message += '2?? ' + dbItem._id.unm + ' was tipped ' + dbItem.count + ' times\n';
                            }
                            if (result.length > 2) {
                                dbItem = result[2];
                                message += '3?? ' + dbItem._id.unm + ' was tipped ' + dbItem.count + ' times\n';
                            }
                        } else {
